...
as values, and then discard the first column as it does not have any meaning on future analysis.
...
## **But, I would recommend Regression tree model (Step 3-d) as the best model to use to explain the label values based on features as it is straight
forward and simple to grasp. Disadvantage of this model over other models is that it is not the best model in terms of accuracy of prediction as shown
above with the calculated test error (MSE).** 
